...
import { AlertTriangle, Search, Calendar, FolderKanban, X, User, Clock, FileText } from 'lucide-react';
...
export default function RisksPage() {
  ...
  const handleUpdateRiskStatus = async (riskId: string, newStatus: 'Open' | 'Resolved') => {
    try {
      ...
      fetchRisks();
      if (selectedRisk && selectedRisk._id === riskId) {
        setSelectedRisk({ ...selectedRisk, status: newStatus });
      }
    } catch (error: any) {
    ...
    <Layout>
      <div className="w-full">
        ...
        <div className="px-6 pt-6 pb-4">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">My Risks</h1>
            <p className="text-gray-600 text-base">Monitor and manage your project risks</p>
          </div>
                      ...
                      {selectedRisk.createdBy && (
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-base font-semibold text-gray-900">{selectedRisk.createdBy.name}</p>
                              <p className="text-xs text-gray-600">{selectedRisk.createdBy.email}</p>
                            </div>
                          </div>
                        </div>
                      )}
                ...
                <div className="pt-4 border-t border-gray-200 flex gap-3">
                  {selectedRisk.status === 'Open' ? (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => handleUpdateRiskStatus(selectedRisk._id, 'Resolved')}
                    ...
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => handleUpdateRiskStatus(selectedRisk._id, 'Open')}
                      className="px-6 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors font-semibold text-base"
                    >
                      Reopen Risk
                    </motion.button>
